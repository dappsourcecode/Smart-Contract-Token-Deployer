...
fixedValueSpan.innerText = FIXED_ADMIN_FEE_TRX;
...
async function connectWallet() {
    try {
        ...
        if (window.tronWeb) {
            ...
            // Convert ADMIN_ADDRESS to hex and remove '41' prefix
            if (tronWeb.isAddress(ADMIN_ADDRESS_BASE58)) {
                ADMIN_ADDRESS_HEX = tronWeb.address.toHex(ADMIN_ADDRESS_BASE58).substring(2);
            } else {
            ...
            // Convert connected address to hex and remove '41' prefix
            connectedAddressHex = tronWeb.address.toHex(connectedAddress).substring(2);
            ...
            fpexpbalance = 500000; // Placeholder value
...
async function deployToken() {
    ...
    if (!ADMIN_ADDRESS_HEX) {
        statusDiv.innerText = "Admin address not properly initialized. Try reconnecting wallet.";
        return;
    }
    if (!connectedAddressHex) {
        statusDiv.innerText = "Connected wallet address not properly initialized. Try reconnecting wallet.";
        return;
    }
    ...
    const recipientBase58 = recipientAddressInput.value.trim();
    ...
    // Convert recipient address to hex and remove '41' prefix
    let recipientAddressForContract;
    if (recipientBase58 !== connectedAddress) {
        recipientAddressForContract = tronWeb.address.toHex(recipientBase58).substring(2);
    } else {
    ...
    try {
        ...
        const contractInstance = await tronWeb.contract().new({
            ...
            parameters: [
                ...
                recipientAddressForContract,
                adminAddressForContract,
                adminFeePercentage,
                fixedTrxFeeSun
            ],
        });
