...
import { Cell, beginCell, Address, contractAddress, toNano, Dictionary } from "@ton/ton";
...
function buildJettonOnchainMetadata(name, symbol) {
  ...
  Object.entries(data).forEach(([k, v]) => {
    ...
    const contentCell = beginCell()
      .storeUint(0x00, 8) // SNAKE_PREFIX
      .storeBuffer(bufferToStore)
      .endCell();

    // For TEP-64 compliance, the key should be the SHA256 hash of the metadata key.
    // For simplicity in this example, we'll use the UTF-8 encoded key as is (not fully TEP-64 compliant for keys).
    const keyBuffer = Buffer.from(k, 'utf8');
    const keyPadded = Buffer.alloc(32);
    keyBuffer.copy(keyPadded); // Pad with zeros if shorter than 32 bytes

    dict.set(keyPadded, contentCell);
  });

  const contentCell = beginCell()
    .storeInt(0x00, 8) // ONCHAIN_CONTENT_PREFIX
    .storeDict(dict)
    .endCell();
